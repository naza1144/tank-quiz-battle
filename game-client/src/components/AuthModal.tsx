...
import { Shield, Sparkles, User, LogIn, Gamepad2 } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ onLogin }) => {
  ...
  const handleGuestLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const name = gamerTag.trim() || `Tanker_${Math.floor(1000 + Math.random() * 9000)}`;
    const guestToken = `guest-${Date.now()}:${name}`;
    onLogin(guestToken, name);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-950 font-thai text-slate-100">
      <div className="relative w-full max-w-md bg-slate-900/90 border-4 border-amber-500 rounded-3xl p-6 sm:p-8 shadow-[0_0_60px_rgba(245,158,11,0.3)] backdrop-blur-md text-center">
        ...
        <p className="text-xs sm:text-sm text-slate-400 mb-6">
          เกมยิงรถถังคลาสสิก 1990 • ตอบโจทย์ Multiple Choice แลกกระสุน
        </p>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full py-3.5 px-4 bg-white hover:bg-slate-100 text-slate-900 font-extrabold text-sm rounded-2xl shadow-lg flex items-center justify-center gap-3 transition-all active:scale-95 border border-slate-200"
          >
          ...
          <div className="flex items-center gap-3 my-4">
            ...
            <span className="text-xs text-slate-500 font-bold">หรือ เล่นทันทีแบบด่วน</span>
            <div className="flex-1 h-px bg-slate-800" />
          </div>

          {/* 2. Quick Guest / Student Tag Form */}
          <form onSubmit={handleGuestLogin} className="space-y-3">
            <div className="relative">
              <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input
                type="text"
                placeholder="พิมพ์ชื่อหรือฉายานักขับ..."
                value={gamerTag}
                onChange={(e) => setGamerTag(e.target.value)}
                maxLength={15}
                className="w-full pl-10 pr-4 py-3 bg-slate-800/80 border border-slate-700 rounded-2xl text-sm font-bold text-white placeholder-slate-500 focus:outline-none focus:border-amber-400 transition-all"
              />
            </div>
            ...
            >
              <Gamepad2 className="w-4 h-4 fill-slate-950" /> เข้าสู่สนามประลอง (Play Now)
            </button>
          </form>
        ...
        <div className="mt-6 pt-4 border-t border-slate-800 text-[11px] text-slate-500 leading-relaxed">
          รองรับการล็อกอินยืนยันตัวตน OIDC / Keycloak ของระบบเดิมอย่างเต็มรูปแบบ
        </div>
