...
export class QuizManager {
  ...
  public getRandomQuestion(category?: string, difficulty?: string): QuizQuestion {
    ...
    if (category && category !== 'ALL') {
      ...
      pool = pool.filter(q => q.category.toUpperCase() === catUpper || q.subjectCode?.toUpperCase() === catUpper);
  ...
  public getAllQuestions(filter?: { category?: string; difficulty?: string; search?: string; source?: string }): QuizQuestion[] {
    ...
    if (filter?.search) {
      ...
      result = result.filter(item => 
        ...
        (item.questionEn && item.questionEn.toLowerCase().includes(q)) ||
  ...
  public updateQuestion(id: string, updates: Partial<QuizQuestion>): QuizQuestion | null {
    ...
    const diff = (updates.difficulty || this.questions[idx].difficulty) as DifficultyLevel;
  ...
  public bulkImport(newQuestions: any[], mode: 'append' | 'replace' = 'append', source: 'LOCAL' | 'EXTERNAL' = 'LOCAL'): { added: number; total: number } {
    const validated: QuizQuestion[] = [];
    for (let i = 0; i < newQuestions.length; i++) {
      const item = newQuestions[i];
      if (item && (item.questionTh || item.question) && Array.isArray(item.options) && item.options.length >= 2) {
        const diff: DifficultyLevel = item.difficulty === 'EASY' || item.difficulty === 'HARD' ? item.difficulty : 'MEDIUM';
        ...
        validated.push({
          ...
          categoryTh: item.categoryTh || item.category || 'โจทย์นำเข้า',
          questionTh: item.questionTh || item.question,
  ...
  public syncFromExternalProvider(providerId: string, questions: any[], replace: boolean = false): { added: number; total: number } {
    if (replace) {
      // Remove previous questions from this provider
      this.questions = this.questions.filter(q => q.source !== 'EXTERNAL' || (q as any).providerId !== providerId);
    }
    const withProvider = questions.map(q => ({
      ...q,
      providerId,
      source: 'EXTERNAL'
    }));
    return this.bulkImport(withProvider, 'append', 'EXTERNAL');
  }
