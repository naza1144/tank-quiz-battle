...
import { GoogleSyncRequest, UserRoleCode } from './types.js';
...
// 1. Health Probe
// -------------------------------------------------------------
app.get('/api/account/health', (_req: Request, res: Response) => {
  res.json({
    status: 'ok',
...
// -------------------------------------------------------------
// 3. Google OAuth User Sync & Token Provisioning
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// 6. Verify Permission (Helper for Inter-Service RBAC Check)
// -------------------------------------------------------------
app.post('/api/account/verify-permission', (req: Request, res: Response) => {
  ...
  const allowed = accountDirectory.hasPermission(role, permission);
  res.json({
    success: true,
    role,
    permission,
